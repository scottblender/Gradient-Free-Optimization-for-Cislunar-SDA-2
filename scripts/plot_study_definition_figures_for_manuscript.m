...
% Manuscript-specific catalog, slot, and visibility figures are generated in
% their final styled state before export. Legacy generation remains only for
% measurement and tracking-case sections. No persistent copy is left in the
% old results/study_definition_figures directory.
...
% Only measurement/cases still use the legacy generator. The figure classes
% that previously suffered export-time layout regressions are generated by
% manuscript-specific renderers before the pure exporter is invoked.
legacySections = requested(~ismember(requested,["catalog","slots","visibility"]));
...
if ismember("catalog",requested)
    outputs.catalog = plot_orbit_catalog_manuscript(inspectFigures,outputDirectory);
end
