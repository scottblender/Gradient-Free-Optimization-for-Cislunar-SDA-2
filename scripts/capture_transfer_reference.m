...
% Authoritative fixed endpoint states for the low-thrust case. These are
% the states used by the original study: old row 52/slot 10 and old
% row 400/slot 1 under linspace(0,T,50).
fixedDepartureState = [ ...
...
assert(norm(fixedDepartureState- ...
    legacyInclusiveDepSlotState) <= 1e-10, ...
    ['Hard-coded departure state does not match old row 52, slot 10. ' ...
     'State error = %.6e.'], ...
    norm(fixedDepartureState-legacyInclusiveDepSlotState));
assert(norm(fixedArrivalState-arrSlotState) <= 1e-10, ...
    ['Hard-coded arrival state does not match old row 400, slot 1. ' ...
     'State error = %.6e.'], ...
    norm(fixedArrivalState-arrSlotState));
