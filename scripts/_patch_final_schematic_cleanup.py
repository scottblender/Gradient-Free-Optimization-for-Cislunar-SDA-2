...
% Choose a representative sensor threshold larger than the physical
% occultation angle so the additional sensor-margin region is visible in
% the schematic. The implemented model still uses
...
% Additional sensor margin between the physical limb and the configured
% keepout boundary. Draw only this annular angular sector in yellow.
marginOuter = linspace(thetaOcc,thetaKeepout,180);
marginInner = fliplr(marginOuter);
innerRadius = 0.10;
patch(ax, ...
    [observer(1)+innerRadius*cos(marginOuter), ...
     observer(1)+sectorRadius*cos(marginOuter), ...
     observer(1)+sectorRadius*cos(marginInner), ...
     observer(1)+innerRadius*cos(marginInner)], ...
    [observer(2)+innerRadius*sin(marginOuter), ...
     observer(2)+sectorRadius*sin(marginOuter), ...
     observer(2)+sectorRadius*sin(marginInner), ...
     observer(2)+innerRadius*sin(marginInner)], ...
...
ptOcc = observer + 0.98*[cos(thetaOcc/2),sin(thetaOcc/2)];
text(ax,-0.90,-0.22,'\\theta_{occ,b}', ...
    'Color',cOcc,'FontWeight','bold','FontSize',16);
plot(ax,[-0.62,ptOcc(1)],[-0.17,ptOcc(2)],'-', ...
    'Color',cOcc,'LineWidth',1.3);

ptKeep = observer + 1.52*[cos(thetaKeepout/2),sin(thetaKeepout/2)];
text(ax,-0.43,0.72,'\\theta_{keepout,b}', ...
    'Color',cSensor,'FontWeight','bold','FontSize',16);
plot(ax,[-0.10,ptKeep(1)],[0.64,ptKeep(2)],'-', ...
    'Color',cSensor,'LineWidth',1.3);

ptB = observer + 2.16*[cos(thetaB/2),sin(thetaB/2)];
text(ax,0.03,1.40,'\\theta_b', ...
    'Color',cTarget,'FontWeight','bold','FontSize',16);
plot(ax,[0.18,ptB(1)],[1.31,ptB(2)],'-', ...
    'Color',cTarget,'LineWidth',1.3);
...
plot(ax,[0.12,ptOccRegion(1)],[-1.14,ptOccRegion(2)],'-', ...
    'Color',cOcc,'LineWidth',1.3);
...
text(ax,0.78,2.05,{'sensor';'margin'}, ...
    ...
    'FontWeight','bold','HorizontalAlignment','center');
plot(ax,[0.78,ptMargin(1)],[1.73,ptMargin(2)],'-', ...
    'Color',cSensor,'LineWidth',1.3);
...
% rho_xy is the horizontal coordinate in this panel, so label it like an
% axis quantity in black. Gray is reserved for the projection guide line.
