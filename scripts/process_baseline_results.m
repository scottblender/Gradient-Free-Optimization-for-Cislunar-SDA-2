...
function ctx = buildObjectiveContextForRun(runInfo, S, baseCtx, cfg)
    ...
    sun_exclusion_deg = getNumericVarValue( ...
        S,["sun_exclusion_deg","sun_min_deg"]);
    moon_exclusion_deg = getNumericVarValue( ...
        S,["moon_exclusion_deg","moon_min_deg"]);
    earth_exclusion_deg = getNumericVarValue( ...
        S,["earth_exclusion_deg","earth_min_deg"]);

    if ~isfinite(sun_exclusion_deg), sun_exclusion_deg = 20; end
    if ~isfinite(moon_exclusion_deg), moon_exclusion_deg = 10; end
    if ~isfinite(earth_exclusion_deg), earth_exclusion_deg = 0; end

    ctx.sun_exclusion = deg2rad(sun_exclusion_deg);
    ctx.moon_exclusion = deg2rad(moon_exclusion_deg);
    ctx.earth_exclusion = deg2rad(earth_exclusion_deg);
