...
assert(departureSlotReferenceError <= 1e-10, ...
    ['Resolved departure does not match old row 52, slot 10. ' ...
     'Slot-state error = %.6e.'],departureSlotReferenceError);
assert(arrivalSlotReferenceError <= 1e-10, ...
    ['Resolved arrival does not match old row 400, slot 1. ' ...
     'Slot-state error = %.6e.'],arrivalSlotReferenceError);
...
assert(departureSlotState(3)<0, ...
    ['Old row 52, slot 10 must lie below the rotating x-y plane. ' ...
...
endpointAudit = table(endpoint,legacyCatalogRow,resolvedCatalogRow, ...
    ...
    slotZ_LU,slotStateReferenceError);

fprintf('\n--- Low-thrust endpoint identity audit ---\n');
disp(endpointAudit);
fprintf('Exact old-row-52 slot-10 state [x y z vx vy vz]:\n');
