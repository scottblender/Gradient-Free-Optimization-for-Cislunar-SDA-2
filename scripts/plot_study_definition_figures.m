...
format_manuscript_legend(ax,legendHandle,style,style.geometryPlotPosition);
...
% Stop the occultation leader just before the gray arc. The arrow still
% identifies the arc without placing its filled head over the arc itself.
...
% Put each leader endpoint well inside the region it describes instead of
% on a nearby boundary line. This keeps the arrowhead visually connected
% to the shaded quantity in both the screen preview and vector EPS export.
...
target = [rhoXY,rho(3)];
quiver(ax2,0,0,4.25,0,0,'Color','k','LineWidth',1.8,'MaxHeadSize',0.08);
quiver(ax2,0,0,0,3.45,0,'Color','k','LineWidth',1.8,'MaxHeadSize',0.08);
...
text(ax2,-0.10,3.74,'z','FontWeight','bold','FontSize',style.fontSize, ...
    'HorizontalAlignment','center','VerticalAlignment','bottom');
...
function fig = publication_figure(widthInches,heightInches,axesFontSize)

style = reviewer2_paper_style();
if nargin<3, axesFontSize=style.fontSize; end
fig = figure( ...
    'Color','w', ...
    'Units','inches', ...
