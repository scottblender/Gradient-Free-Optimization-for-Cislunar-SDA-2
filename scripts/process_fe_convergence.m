...
for k = 1:n
    ...
    try
        ...
        assert(isfield(r,'parallelOverflowEvaluations') && ...
            isfinite(r.parallelOverflowEvaluations) && ...
            r.parallelOverflowEvaluations >= 0 && ...
            r.parallelOverflowEvaluations == round(r.parallelOverflowEvaluations), ...
            'Invalid parallel-overflow metadata.');
        assert(isfield(r,'budgetRuntime_s') && isfinite(r.budgetRuntime_s) && ...
            r.budgetRuntime_s >= 0 && ...
            isfield(r,'solverWallRuntime_s') && isfinite(r.solverWallRuntime_s) && ...
            r.solverWallRuntime_s >= r.budgetRuntime_s && ...
            abs(r.runtime_s-r.budgetRuntime_s) <= ...
                1e-10*max(1,abs(r.budgetRuntime_s)), ...
            'Invalid budget/wall runtime metadata.');

        opt = string(r.optimizer);
        if opt == "BAYESIAN"
            overflow = r.parallelOverflowEvaluations;
            assert(isfield(r,'postSearchFunctionEvaluations') && ...
                r.postSearchFunctionEvaluations == overflow && ...
                r.solverFunctionEvaluations == budget+overflow, ...
                'Parallel Bayesian solver calls do not match admitted FE plus overflow.');
            if isfield(r.settings,'useParallel') && r.settings.useParallel && ...
                    isfield(r.settings,'workerCount') && r.settings.workerCount > 0
                assert(overflow <= r.settings.workerCount, ...
                    'Parallel Bayesian overflow exceeds the worker count.');
            end
        else
            expectedPostSearch = double(opt == "GA");
            assert(r.parallelOverflowEvaluations == 0 && ...
                isfield(r,'postSearchFunctionEvaluations') && ...
    ...
    for a = 1:numel(optimizers)
        ...
        for j = 1:numel(idx)
            ...
            identity = table(key,opt,r.optimizerSeed,r.bestJ,r.nEvaluations, ...
                ...
                r.parallelOverflowEvaluations,r.runtime_s,r.budgetRuntime_s, ...
                r.solverWallRuntime_s,r.validationRuntime_s, ...
                inventory.run_file(idx(j)), ...
