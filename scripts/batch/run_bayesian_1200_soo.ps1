...
# Dedicated Reviewer 2 Bayesian study. Keep this deliberately focused so
# bayesopt surrogate/acquisition overhead remains tractable.
$StudyId = "reviewer2_bayesian_1200_v1"
$StudyFolder = "BAYESIAN_1200"
$Alg = "BAYESIAN"
$EvalBudget = 1200
...
$MissionRoot = Join-Path (Join-Path (Join-Path $StudyRoot "runs_BAYESIAN") "ao") "lg"
...
$TotalRuns = $Seeds.Count
$CompletedRuns = 0
foreach ($seed in $Seeds) {
    $CompletedRuns++
    $seedCode = $seed.ToString("000")
    $runName = "bo1200_ao_o3_p1_seed$($seedCode)"
    $runDir = Join-Path $MissionRoot $runName

    $percent = [math]::Round(100 * $CompletedRuns / $TotalRuns, 1)
    Write-Progress -Activity "Bayesian 1200-FE study" `
        -Status "$CompletedRuns of $TotalRuns | Lunar Gateway | seed $seed" `
        -PercentComplete $percent

    Write-Host "`nBayesian 1200 FE: [LUNAR_GATEWAY] [ANGLES_ONLY] $runName"
    Write-Host "Observers: 3 | periods: 1 | FE budget: 1200"
    Write-Host "Optimizer seed: $seed | measurement seed: $MeasurementNoiseSeed"

    Invoke-MatlabRun -RunDir $runDir -Seed $seed
}

Write-Progress -Activity "Bayesian 1200-FE study" -Completed
Write-Host "`nBayesian 1200-FE runs complete."
Write-Host "Bayesian study -> $StudyRoot"
# ---------------------------------------------------------
