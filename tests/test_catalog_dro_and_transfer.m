...
EXPECTED_ARR_SLOT = 1;
EXPECTED_DEP_ID   = "northern_halo_l1:1015";
EXPECTED_ARR_ID   = "southern_halo_l2:97";
EXPECTED_LEGACY_DEP_ROW = 51;
EXPECTED_DEP_STATE0 = [ ...
...
assert(families(depIndex) == "NHL1", ...
    "Departure orbit must belong to the NHL1 family.");
assert(T.state{depIndex}(1,3) > 0 && ...
    mean(T.state{depIndex}(:,3)) > 0, ...
    "Departure orbit is not on the northern L1 branch.");
assert(norm(T.state{depIndex}(1,:)-EXPECTED_DEP_STATE0) <= ...
    STATE_TOLERANCE, ...
    "Departure does not match old catalog row 51.");
if isfield(transferRef.dep,"legacyIndex")
    assert(transferRef.dep.legacyIndex == EXPECTED_LEGACY_DEP_ROW, ...
        "Stored legacy departure row must be 51, not the southern row 52.");
end
