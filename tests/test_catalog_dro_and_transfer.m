...
legacyCatalogPath = fullfile( ...
    projectPaths.data, "JPL_CR3BP_OrbitCatalog_Old.mat");
...
assert(isfile(legacyCatalogPath), ...
    ['The old catalog is required to verify row 52, slot 10:\n%s'], ...
    legacyCatalogPath);
...
assert(isfield(transferRef,"slotsPerOrbit") && ...
    transferRef.slotsPerOrbit == SLOTS_PER_ORBIT, ...
    "Transfer reference contains the wrong number of slots.");
assert(isfield(transferRef,"targetStateDefinition") && ...
    string(transferRef.targetStateDefinition) == ...
    "explicit_fixed_states_v1", ...
    "Low-thrust target does not use explicit fixed endpoint states.");
assert(isfield(transferRef.dep,"transferState") && ...
    isfield(transferRef.arr,"transferState"), ...
    "Transfer reference is missing fixed endpoint states.");

% Independently reconstruct the endpoint ICs from the old database.
legacyData = load(legacyCatalogPath,"T");
legacyT = legacyData.T;
legacyPeriods = legacyT.("Period (TU) ");

legacyDepState0 = legacyT.state{EXPECTED_LEGACY_DEP_ROW}(1,:);
legacyArrState0 = legacyT.state{EXPECTED_LEGACY_ARR_ROW}(1,:);
assert(norm(legacyDepState0-EXPECTED_DEP_STATE0) <= STATE_TOLERANCE, ...
    "The expected row-52 orbit IC does not match the old catalog.");
assert(norm(reshape(transferRef.dep.state0,1,[])- ...
    legacyDepState0) <= STATE_TOLERANCE, ...
    "Captured departure orbit IC is not old catalog row 52.");
assert(norm(reshape(transferRef.arr.state0,1,[])- ...
    legacyArrState0) <= STATE_TOLERANCE, ...
    "Captured arrival orbit IC is not old catalog row 400.");

legacyDepSlotEpoch = (EXPECTED_DEP_SLOT-1)* ...
    legacyPeriods(EXPECTED_LEGACY_DEP_ROW)/SLOTS_PER_ORBIT;
legacyArrSlotEpoch = (EXPECTED_ARR_SLOT-1)* ...
    legacyPeriods(EXPECTED_LEGACY_ARR_ROW)/SLOTS_PER_ORBIT;

legacyDepSlotState = reconstruct_state( ...
    legacyT.time{EXPECTED_LEGACY_DEP_ROW}, ...
    legacyT.state{EXPECTED_LEGACY_DEP_ROW}, ...
    legacyDepSlotEpoch);
legacyArrSlotState = reconstruct_state( ...
    legacyT.time{EXPECTED_LEGACY_ARR_ROW}, ...
    legacyT.state{EXPECTED_LEGACY_ARR_ROW}, ...
    legacyArrSlotEpoch);

assert(norm(reshape(transferRef.dep.slotState,1,[])- ...
    legacyDepSlotState) <= SLOT_TOLERANCE, ...
    "Captured departure slot IC is not old row 52, slot 10.");
assert(norm(reshape(transferRef.arr.slotState,1,[])- ...
    legacyArrSlotState) <= SLOT_TOLERANCE, ...
    "Captured arrival slot IC is not old row 400, slot 1.");

legacyTransferDepEpoch = (EXPECTED_DEP_SLOT-1)* ...
    legacyPeriods(EXPECTED_LEGACY_DEP_ROW)/(SLOTS_PER_ORBIT-1);
legacyTransferDepState = reconstruct_state( ...
    legacyT.time{EXPECTED_LEGACY_DEP_ROW}, ...
    legacyT.state{EXPECTED_LEGACY_DEP_ROW}, ...
    legacyTransferDepEpoch);

assert(norm(EXPECTED_TRANSFER_DEP_STATE- ...
    legacyTransferDepState) <= SLOT_TOLERANCE, ...
    "Hard-coded departure IC is not old row 52 at 9T/49.");
assert(norm(EXPECTED_TRANSFER_ARR_STATE- ...
    legacyArrSlotState) <= SLOT_TOLERANCE, ...
    "Hard-coded arrival IC is not old row 400 at t=0.");
assert(norm(reshape(transferRef.dep.transferState,1,[])- ...
    EXPECTED_TRANSFER_DEP_STATE) <= SLOT_TOLERANCE, ...
    "Stored fixed departure IC changed.");
assert(norm(reshape(transferRef.arr.transferState,1,[])- ...
    EXPECTED_TRANSFER_ARR_STATE) <= SLOT_TOLERANCE, ...
    "Stored fixed arrival IC changed.");

assert(string(transferRef.dep.orbitID) == EXPECTED_DEP_ID, ...
    "Departure orbit changed from %s to %s.", ...
    EXPECTED_DEP_ID,string(transferRef.dep.orbitID));
assert(string(transferRef.arr.orbitID) == EXPECTED_ARR_ID, ...
    "Arrival orbit changed from %s to %s.", ...
    EXPECTED_ARR_ID,string(transferRef.arr.orbitID));
...
assert(families(depIndex) == "SHL1", ...
    "Departure orbit must belong to the SHL1 family.");
assert(T.state{depIndex}(1,3) < 0 && ...
    mean(T.state{depIndex}(:,3)) < 0, ...
    "Departure orbit is not on the southern L1 branch.");
assert(norm(T.state{depIndex}(1,:)-EXPECTED_DEP_STATE0) <= ...
    STATE_TOLERANCE, ...
    "Departure does not match old catalog row 52.");
assert(isfield(transferRef.dep,"legacyIndex") && ...
    transferRef.dep.legacyIndex == EXPECTED_LEGACY_DEP_ROW, ...
    "Stored legacy departure row must be 52.");
validArrivalFamilies = ["SHL2","SNRHL2"];
assert(ismember(families(arrIndex),validArrivalFamilies), ...
    ['Arrival orbit must belong to a southern L2 halo or ' ...
     'near-rectilinear halo family. Catalog family = %s.'], ...
    families(arrIndex));
...
for i = 1:numel(transferIndices)
    ...
    if i == 1
        endpointReference = transferRef.dep;
    else
        endpointReference = transferRef.arr;
    end

    assert(isfield(endpointReference,"slotState") && ...
        isfield(endpointReference,"slotEpoch"), ...
        "%s reference is missing its captured slot IC.", ...
        transferLabels(i));
    ...
    interpolant = griddedInterpolant( ...
        tUnique,sRaw(uniqueIndex,:),"spline");
    reconstructedSlotState = ...
        interpolant(slotTimes(endpointReference.slot));
    slotReferenceError = norm( ...
        reconstructedSlotState- ...
        reshape(endpointReference.slotState,1,[]));

    assert(abs(endpointReference.slotEpoch- ...
        slotTimes(endpointReference.slot)) <= ...
        100*eps(period), ...
        "%s captured slot epoch is inconsistent.", ...
        transferLabels(i));
    assert(slotReferenceError <= SLOT_TOLERANCE, ...
        ['%s slot IC is inconsistent with the captured old-catalog ' ...
         'row and slot. State error = %.6e.'], ...
        transferLabels(i),slotReferenceError);
end
