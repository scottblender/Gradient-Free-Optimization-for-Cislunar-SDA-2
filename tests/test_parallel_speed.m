...
for r = 1:nRepeats
    ...
    for m = order
        ...
        benchmark = struct('mission',missionType,'budget',budget,'nRepeats',nRepeats, ...
            'optimizerSeed',0,'measurementSeed',1001,'environment',environment, ...
            'results',results,'histories',{completed},'complete',row==nRows);
...
summary = sprintf(['GA %s | %d FE | %d fixed-seed timing repetitions per mode\n' ...
    'MATLAB: %s | Platform: %s | Detected cores: %d\n' ...
    'Mean parallel workers: %.1f\n' ...
    ...
    missionType,budget,nRepeats,environment.matlabVersion,environment.computer, ...
    environment.detectedCores,mean(results.Workers(parallel)), ...
...
function environment = benchmark_environment()
environment = struct();
environment.matlabVersion = string(version);
environment.release = string(version('-release'));
environment.computer = string(computer);
try
    environment.detectedCores = feature('numcores');
catch
    environment.detectedCores = NaN;
end
environment.hostProcessorCount = string(getenv('NUMBER_OF_PROCESSORS'));
