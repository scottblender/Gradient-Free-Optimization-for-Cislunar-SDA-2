...
for k = 1:4
    ...
    if k==2 || k==3
        ...
        expectedBottom=min(ap(2)+ap(4)+topClearance+style.geometryLegendGap, ...
            0.99-lp(4));
        assert(abs(lp(2)-expectedBottom)<0.01, ...
            'Export-time legend placement did not preserve the shared moderate gap.');
...
assert(style.geometryLegendGap>=0.006 && style.geometryLegendGap<=0.010 && ...
    style.legendTopPadding<=0.020 && style.legendMinimumTopClearance<=0.012, ...
    'Legend spacing should stay moderately close to the plot without crowding it.');
...
fprintf(['Manuscript EPS canvas, contained 3-D geometry, manual/automatic sparse ' ...
    '3-D ticks, preserved moderate legend spacing, optimizer spacing, abbreviation, ' ...
