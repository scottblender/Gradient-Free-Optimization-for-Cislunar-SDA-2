...
for k = 1:4
    ...
    if k==2 || k==3
        ...
        assert(strcmpi(lgd.Location,'none'), ...
            'Export reset the manually positioned legend to an automatic location.');
        expectedBottom=min(ap(2)+ap(4)+style.legendAxesGap,0.99-lp(4));
        assert(abs(lp(2)-expectedBottom)<0.01, ...
            'Export-time legend placement did not preserve the explicit axes gap.');
        ...
        assert(finalGap>0 && finalGap<=style.legendAxesGap+0.01, ...
            'Final legend gap is outside the intended range.');
...
assert(style.legendAxesGap>=0.008 && style.legendAxesGap<=0.012 && ...
    style.geometryLegendGap==style.legendAxesGap && style.legendTopPadding<=0.020, ...
    'Legend spacing should use one moderate explicit axes-to-legend gap.');
...
fprintf(['Manuscript EPS canvas, contained 3-D geometry, manual/automatic sparse ' ...
    '3-D ticks, manual legend placement, optimizer spacing, abbreviation, legend, ' ...
