...
assert(isfield(transferRef,'targetStateDefinition') && ...
    ...
    'Transfer reference must use explicit fixed endpoint states.');
...
assert(isfield(info,'departureStateSource') && ...
    string(info.departureStateSource) == "FIXED_STATE", ...
    'The solver did not use the fixed departure IC.');
assert(isfield(info,'x_dep') && ...
    norm(info.x_dep(:)-transferRef.dep.transferState(:)) <= 1e-12, ...
    'The solver departure IC differs from the stored fixed state.');
assert(isfield(info,'x_arr') && ...
    norm(info.x_arr(:)-transferRef.arr.transferState(:)) <= 1e-12, ...
    'The solver arrival IC differs from the stored fixed state.');
