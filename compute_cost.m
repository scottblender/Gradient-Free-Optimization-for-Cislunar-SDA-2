function [J_total, J_1, J_2, J_3] = compute_cost(s_act, s_ekf, cov, stabilities_vec, opt_flag, costFlags)
    ...
    weights = [1000, 1000, 10];  % component weights

    % --- J1: State RMSE  ---
    err  = s_act - s_ekf;
    rmse = sqrt(mean(sum(err.^2,2)));
    rmse = max(rmse, realmin);
    p1 = 0.5;
    J1_raw = weights(1) * rmse^(p1);
    ...
    det_term = mean(det_vals);   
    det_term = max(det_term, realmin);       % safety
    p2 = 0.05;
    J2_raw   = weights(2) * det_term^(p2);
