...
for k = 2:num_steps
    ...
    for i = 1:num_obs   
        ...
        noise   = mvnrnd([0;0], R)';   % ok: 2x1
        z_meas  = z_clean + noise;

        % predicted measurement
        z_pred  = measurement_model(x_upd(1:3), r_obs);
        y_tilde = z_meas - z_pred;
        y_tilde(1) = atan2(sin(y_tilde(1)), cos(y_tilde(1))); % wrap angle

        H = measurement_jacobian(x_upd(1:3), r_obs); % 2x6

        S = H * P_upd * H' + R;  % 2x2

        % Robust gain computation + Joseph covariance update
        [Rchol,p] = chol(S);
        if p == 0
            % K = (P*H')/S using Cholesky solves
            PHt = P_upd * H';                 % 6x2
            K   = (PHt / Rchol) / Rchol';     % 6x2
        else
            % fallback if S not PD
            K = (P_upd * H') * pinv(S);
        end

        x_upd = x_upd + K * y_tilde;

        P_upd = (I6 - K*H) * P_upd * (I6 - K*H)' + K*R*K';  % Joseph form
        P_upd = (P_upd + P_upd')/2;
    end
